"""
EXPERIMENT_003: Training with Tversky (alpha=0.7, beta=0.3) + CrossEntropy (bg=1.0)
Goal: Reduce severe False Positive over-segmentation while preserving high recall.
"""
...
class TverskyCELoss(nn.Module):
    def __init__(self, tversky_weight=0.5, ce_weight=0.5, alpha=0.7, beta=0.3, class_weights=None):
        super().__init__()
    ...
    def forward(self, logits, targets):
        loss_tv = self.tversky(logits, targets.unsqueeze(1))
        loss_ce = self.ce(logits, targets.long())
        return self.tversky_weight * loss_tv + self.ce_weight * loss_ce
...
def train(args):
    ...
    print(f'=== EXPERIMENT_003 Training (Tversky alpha={args.alpha}, beta={args.beta}) ===', flush=True)
    ...
    cases_per_epoch = args.cases_per_epoch
    ...
    # EXACT class weights approved for Experiment_003: [1.0, 1.0, 2.0, 1.0, 3.0, 3.0]
    class_weights = torch.tensor([1.0, 1.0, 2.0, 1.0, 3.0, 3.0], dtype=torch.float32).to(device)
    print(f'Class weights: {class_weights.tolist()} (bg_weight=1.0)', flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if is_best and run_val:
            ...
            print(f'  *** [BEST CHECKPOINT SAVED] Ep{epoch:03d} Full-Vol Val Dice: {best_val_dice:.4f} ***', flush=True)
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='EXPERIMENT_003 Training with Tversky Loss')
    p.add_argument('--data_dir',           default='data/oaizib')
    p.add_argument('--splits_dir',         default='splits')
    p.add_argument('--checkpoints_dir',    default='experiments/experiment_003/checkpoints')
    p.add_argument('--logs_dir',           default='experiments/experiment_003/logs')
